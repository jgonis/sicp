...
(define-library (sicp ex323)
  ...
  (begin
     (define (make-deque)
       (cons '() '())) 
     (define (empty-deque? dek)
       (null? (front-ptr dek))) 
     (define (front-deque dek)
       (cond ((empty-deque? dek) (error "Tried to get front of empty deque!"))
             (else (front-ptr dek)))) 
     (define (rear-deque dek)
       (cond ((empty-deque? dek) (error "Tried to get end of empty deque!"))
             (else (rear-ptr dek))))  
     (define (front-insert-deque! dek item)
       (let ((new-item (cons item '())))
         (cond ((empty-deque? dek) (set-front-ptr! dek new-item)
                (set-rear-ptr! dek new-item)
                dek)
               (else (set-cdr! new-item (front-ptr dek))
                     (set-front-ptr! dek new-item)
                     dek)))) 
