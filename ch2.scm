...
(define-library (sicp generic-lib)
  ...
  (begin
    (define func-list '())
    (define (get-func-list) func-list)
    (define (get-func op type)
...
(define (sicp complex-num-lib-polarv2)
  ...
  (define (install-polar-package)
    ...
    (put-func 'make-from-real-imag 'polar (lambda (x y)
                                            (tag (make-from-real-imag x y))))
    (put-func 'make-from-mag-ang 'polar (lambda (r a) (tag (make-from-mag-ang r a))))
    'done))
...
(define-library (sicp complex-num-lib-v2)
  ...
  (import (scheme base))
  (begin
    (define (add-comple z1 z2)
      (make-from-real-imag (+ (jreal-part z1) (jreal-part z2))
                           (+ (jimag-part z1) (jimag-part z2))))
    (define (sub-complex z1 z2)
      (make-from-real-image (- (jreal-part z1) (jreal-part z2))
                            (- (jimag-part z1) (jimag-part z2))))
    (define (mul-complex z1 z2)
      (make-from-mag-angle (* (jmagnitude z1) (jmagnitude z2))
                           (+ (jangle z1) (jangle z2))))
    (define (div-complex z1 z2)
